...
def offline_score(ts, pattern_length, nms, cl_s, normalize):
    ...
    for index_name in range(len(nms)):
        if torch.cuda.is_available():
            join = stumpy.gpu_stump(ts, pattern_length, nms[index_name].subseq, ignore_trivial=False,
                                    normalize=normalize)[:, 0]
        else:
            join = stumpy.stump(ts, pattern_length, nms[index_name].subseq, ignore_trivial=False, normalize=normalize)[
                   :, 0]
        join = np.array(join)
...
def backward_anomaly2(ts, pattern_length, nm_subseq, normalize, device_id=0):
    ...
    if torch.cuda.is_available():
        score = stumpy.gpu_stump(ts_n, pattern_length, nm_subseq, device_id=device_id, ignore_trivial=False,
                                 normalize=False)[:, 0]
    else:
...
def backward_anomaly_changing_point(ts, pattern_length, nms, normalize, device_id=0):
    ...
    for nm in nms:
        if torch.cuda.is_available():
            scores.append(stumpy.gpu_stump(ts_n, pattern_length, nm.subseq, device_id=device_id, ignore_trivial=False,
                                           normalize=False)[:, 0])
        else:
...
def align_score(NMs, scores, cl_s, slidingWindow):
    ...
    rev_scores = np.array(
        [rev_scores[0]] * ((slidingWindow - 1) // 2) + list(rev_scores) + [rev_scores[-1]] * ((slidingWindow - 1) // 2))
