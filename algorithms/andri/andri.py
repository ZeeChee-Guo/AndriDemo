...
from algorithms.andri.util_andri import *
from algorithms.andri.ahc import membership, adaptive_ahc
from algorithms.andri.plot_andri import *
        ...
        if len(self.seq) > self.W:
            if self.prepare_add:
                ...
                if len(self.seq) > self.W and nm.active == True and np.mean(self.mem[nm_idx]) > nm.nu:
                    if nm.tau < np.mean(self.dist[nm_idx]) + 3 * np.std(self.dist[nm_idx]) and nm.std > np.std(
                            self.dist[nm_idx]):
                        print(
                            f'[{idx}], NM {i}, tau: {nm.tau:.2f} (M: {nm.m}, STD: {nm.std}) vs. {np.mean(self.dist[nm_idx]) + 3 * np.mean(self.dist[nm_idx])}, (M: {np.mean(self.dist[nm_idx])}, STD: {np.mean(self.dist[nm_idx])})')
                        return_status.append(status_window.INACTIVE)
...
class AnDri():
    ...
    def __training(self, training_len, stepwise, min_size=0.025):
        ...
        if self.y is not None:
            n_subseq, n_label = divide_subseq(x_train, self.pattern_length, self.nm_len, overlap=self.overlap,
                                              label=y_train)
        else:
            n_subseq = divide_subseq(x_train, self.pattern_length, self.nm_len, overlap=self.overlap)
        ...
        listcluster, Z, m_subseq, m_tau, m_nu, _, __cached__, d_ci, d_c_std, Ws, _ = adaptive_ahc(
            seqs_n, linkage_method=self.linkage_method, th_reverse=self.th_reverse, kadj=self.kadj, eta=self.eta,
            max_W=self.max_W, min_size=min_size)
        ...
        ## Normal Models
        self.NMs = []
        for m_s, m_t, m_n, m_m, m_std in zip(m_subseq, m_tau, m_nu, d_ci, d_c_std): self.NMs.append(
            NormalModel(m_s, m_t, m_n, m_m, m_std))
        ...
        cl_list = np.array([])
        for lc in listcluster: cl_list = np.append(cl_list, np.ones(
            int(self.pattern_length * self.nm_len * (1 - self.overlap))) * lc)
        ...
        for i in range(0, len(x_train), self.pattern_length):
            ...
            if stepwise:
                if i >= self.pattern_length:
                    x_tests = x_train[i - self.pattern_length:i + self.pattern_length]
                else:
                    x_tests = x_train[i:i + self.pattern_length * 2]
                ...
                if i >= self.pattern_length:
                    ...
                    self.scores = np.append(self.scores, rev_score[:self.pattern_length // 2])
            else:
                ## Find sub-subseq. of anomalies
                if score > selected_th:
                    tmp_score = backward_anomaly(x_train[i - self.pattern_length:i + self.pattern_length],
                                                 self.pattern_length, self.NMs[nm_idx].subseq, self.normalize)
                    ...
                    if before_status:
                        ...
                        self.scores = np.append(self.scores,
                                                np.ones(self.pattern_length) * score / self.NMs[nm_idx].tau)
                    before_status = False
    ...
    def fit(self, X, y=None, online=True, delta=0, training_len=None, stepwise=False, align=True, min_size=0.025):
        ...
        if online:
            ...
            while (self.seq_idx < len(X)):
                ...
                if status_window.ADD in rsts:
                    new_NM, d_nm, new_len = self.add_new_NM(
                        self.ts[self.seq_idx - movingWin.W * self.pattern_length:self.seq_idx + self.pattern_length],
                        self.nm_len, self.overlap, movingWin.NM, self.seq_idx)
                    if new_NM is not None:
                        ...
                        self.dist_nm.append([])
                        print(
                            f'[==> Add NM]: NM# {len(self.NMs) - 1}, TAU: {new_NM.tau}, Nu: {new_NM.nu}, LEN: {new_len}')

                        if self.REVISE_SCORE: self.revise_scores(movingWin, delta, len(self.NMs) - 1, stepwise,
                                                                 status_window.ADD)
                ...
                if stepwise:
                    ...
                    selected_th = selected_nm.tau
                    if score > selected_th:
                        tmp_score = backward_anomaly(
                            self.ts[self.seq_idx - self.pattern_length:self.seq_idx + self.pattern_length],
                            self.pattern_length, selected_nm.subseq, self.normalize)
                        ...
                        if before_status:
                            ...
                            self.scores = np.append(self.scores,
                                                    np.ones(self.pattern_length) * score / movingWin.NM[nm_idx].tau)
                        before_status = False
        ...
        if align:
            ...
            self.scores = np.array(
                [self.scores[0]] * ((self.pattern_length - 1) // 2) + list(self.scores) + [self.scores[-1]] * (
        ...
        while (start_idx < self.seq_idx):
            ...
            if added == status_window.ACTIVE:
                ...
                if min_j == curr_cl:
                    ...
                    rev_score, before_status = self.update_score(movingWin, start_idx, min_j, min_dist, stepwise,
                                                                 before_status)
                    self.scores[
                    start_idx - self.pattern_length // 2 - 1:start_idx - self.pattern_length // 2 - 1 + self.pattern_length] = rev_score[
                                                                                                                               :self.pattern_length]

            elif added == status_window.ADD:
                new_dist = np.linalg.norm(
                    compute_diff_dist(movingWin.NM[nm_ids].subseq, self.ts[start_idx:start_idx + self.pattern_length]))
                ...
                m_t, d_t = membership(movingWin.NM[nm_ids].subseq, self.ts[start_idx:start_idx + self.pattern_length],
                                      movingWin.NM[nm_ids].tau, eta=1)
    ...
    def add_new_NM(self, t_seq, step, overlap, NMs, idx):
        ...
        listcluster, Z, m_subseq, m_tau, m_nu, _, _, d_ci, d_c_std, _, _ = adaptive_ahc(
            win_seqs_n, linkage_method='ward', th_reverse=10, kadj=1, eta=self.eta, max_W=self.max_W, isTrain=False,
            NMs=NMs)
