...
from algorithms.andri.util_andri import intra_cluster_dist, compute_diff_dist, longest_consecutive_sequence
import copy
import math
...
def __compute_dist_cls(seqAs, seqBs, linkage_method='average'):
    ...
    if linkage_method in ['average', 'complete', 'single']:
        ...
        d_res = np.linalg.norm(compute_diff_dist(np.mean(seqAs, axis=0), np.mean(seqBs, axis=0)))
    ...
    if kadj == 1:
        for i in range(len(d_mat)):
            if i != left and i != right and (__min_dist_cl(cluster_new[left], cluster_new[i])) == 1:
                d_mat[left, i] = d_mat[i, left] = __compute_dist_cls([seqs[cl] for cl in cluster_new[left]],
                                                                     [seqs[cl] for cl in cluster_new[i]],
                                                                     linkage_method)
...
def flipped_merge(seqs, left, right, c1, c2, c3, d_c1, d_c2, Z, d_mat, clusters, add_pair, num_seq, th_reverse, kadj,
                  linkage_method):
    """
    ...
    while (d_c1 - c3 > d_c1 * th_reverse / 100 or d_c2 - c3 > d_c2 * th_reverse / 100):
        ...
        if kadj == 1:
            ...
            for cl in clusters:
                d_add = __compute_dist_cls([seqs[id] for id in cl], [seqs[id] for id in brk_l_cls], linkage_method) if (
                                                                                                                           __min_dist_cl(
                                                                                                                               cl,
                                                                                                                               brk_l_cls)) == 1 else MAX_DIST
            ...
            ## Add brk_R
            for cl in clusters:
                d_add = __compute_dist_cls([seqs[id] for id in cl], [seqs[id] for id in brk_r_cls], linkage_method) if (
                                                                                                                           __min_dist_cl(
                                                                                                                               cl,
                                                                                                                               brk_r_cls)) == 1 else MAX_DIST
            ...
            d_all_L, d_all_R = np.array([]), np.array([])
            for cl in clusters:
                d_all_L = np.append(d_all_L, __compute_dist_cls([seqs[id] for id in cl], [seqs[id] for id in brk_l_cls],
                                                                linkage_method))
            d_mat = np.insert(d_mat, brk_idx, d_all_L, axis=0)
            d_all_L = np.insert(d_all_L, brk_idx, MAX_DIST)
            d_mat = np.insert(d_mat, brk_idx, d_all_L, axis=1)
            ...
            ## Add brk_R
            for cl in clusters:
                d_all_R = np.append(d_all_R, __compute_dist_cls([seqs[id] for id in cl], [seqs[id] for id in brk_r_cls],
                                                                linkage_method))
            d_mat = np.insert(d_mat, brk_idx + 1, d_all_R, axis=0)
            d_all_R = np.insert(d_all_R, brk_idx + 1, MAX_DIST)
            d_mat = np.insert(d_mat, brk_idx + 1, d_all_R, axis=1)
        ...
        c3, c4 = __compute_dist_cls([seqs[id] for id in res_cls], [seqs[id] for id in brk_cls], linkage_method), len(
            res_cls) + len(brk_cls)
...
def adaptive_ahc(seqs, linkage_method='ward', th_reverse=5, kadj=1, eta=1, max_W=20, max_nu=0.9, min_size=0.025,
                 isTrain=True, NMs=None):
    """
    ...
    while len(clusters) > 1:
        ...
        if d_c1 - c3 > d_c1 * th_reverse / 100 or d_c2 - c3 > d_c2 * th_reverse / 100:
            ...
            if __min_dist_cl(clusters[left], clusters[right]) < max(kadj, max_W):
                left, right, c1, c2, c3, add_idx, d_mat, clusters, Z, add_pair = flipped_merge(seqs, left, right, c1,
                                                                                               c2, c3, d_c1, d_c2, Z,
                                                                                               d_mat, clusters,
                                                                                               add_pair, num_seq,
                                                                                               th_reverse, kadj,
    ...
    for i in range(1, int(np.max(listcluster)) + 1):
        ...
        min_cl_size = np.sum(cl_lengths) * min_size if isTrain else np.sum(cl_lengths) * np.mean(
            [nm_t.nu for nm_t in NMs])
